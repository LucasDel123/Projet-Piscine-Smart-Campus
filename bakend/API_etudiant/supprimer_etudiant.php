...
$stmt = $db->prepare("DELETE FROM etudiant WHERE id_etudiant = ?");

try {
    $stmt->execute([$_GET['id']]);
    jsonResponse(["message" => "Étudiant supprimé avec succès"], 200);
} catch (PDOException $e) {
    jsonResponse(["error" => "Impossible de supprimer cet étudiant (il a probablement des notes ou inscriptions liées)."], 409);
}
