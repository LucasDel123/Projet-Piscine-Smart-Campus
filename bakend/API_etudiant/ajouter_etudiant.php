...
$stmtCheck = $db->prepare("SELECT id_etudiant FROM etudiant WHERE email = ?");
...
$stmt = $db->prepare("INSERT INTO etudiant (num_etudiant, nom, prenom, email, mdp, filiere, niveau, date_inscription) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
...
try {
    ...
    jsonResponse(["message" => "Étudiant créé avec succès", "id" => $db->lastInsertId()], 201);
